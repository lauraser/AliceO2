...
void PID::initializeHistograms()
{
  ...
  for (size_t idEdxType = 0; idEdxType < rocNames.size(); ++idEdxType) {
    ...
    mMapHist["hMIPNclVsTglSub"].emplace_back(std::make_unique<TH2F>(fmt::format("hMIPNclVsTglSub_{}", name).data(), (fmt::format("sub-thrs. clusters {}", name) + ";#tan(#lambda);sub-thrs. clusters").data(), 50, -2, 2, 20, 0, 20.q));
    if (mCreateCanvas) {
